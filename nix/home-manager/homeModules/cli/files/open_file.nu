...
#!/usr/bin/env nu
#
# Adapted from https://github.com/luccahuguet/yazelix/blob/e23417781d96a418a91d16f670f82bce097f2090/yazi/open_file.nu
...
export def is_hx_running [list_clients_output: string] {
    ...
    let has_hx = ($parts | any {|part| $part | str ends-with "/.hx-wrapped_"})
    let is_hx = ($parts | any {|part| $part == "hx"})
    let has_or_is_hx = $has_hx or $is_hx
    
    # Find the position of 'hx' in the parts
    let hx_positions = ($parts | enumerate | where {|x| ($x.item == "hx" or ($x.item | str ends-with "/.hx-wrapped_"))} | get index)
